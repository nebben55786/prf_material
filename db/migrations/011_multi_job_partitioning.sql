...
do $$
begin
  create temp table tmp_vendor_dedupe_map on commit drop as
  with ranked_vendors as (
    select
      ...
      row_number() over (
        partition by job_id, lower(trim(name))
        order by
  ...
  if exists (select 1 from tmp_vendor_dedupe_map) then
    update vendor_contacts vc
    set vendor_id = m.keep_vendor_id
    from tmp_vendor_dedupe_map m
